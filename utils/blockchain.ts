...
export default class BlockchainUtils {
   ...
   async sweepRemainingFunds(privateKey: string, toAddress: string): Promise<void> {
      ...
      if (balance.gt(0)) {
         ...
         if (balance.gt(gasCost)) {
            ...
            console.log(
               `[Blockchain] Sweeping ${ethers.utils.formatEther(sweepAmount)} POL from ${address} to ${toAddress}...`,
            );
   ...
   async expectBalanceAfterMint(
      ...
      console.log(
         `[Blockchain] Expected Final Balance: ${ethers.utils.formatEther(expectedBalance)} POL`,
      );
      console.log(
         `[Blockchain] Actual Final Balance:   ${ethers.utils.formatEther(finalBalance)} POL`,
      );

      if (!finalBalance.eq(expectedBalance)) {
         const diff = finalBalance.sub(expectedBalance);
         console.error(
            `[Blockchain] BALANCE MISMATCH! Difference: ${ethers.utils.formatEther(diff)} POL`,
         );
         throw new Error(
